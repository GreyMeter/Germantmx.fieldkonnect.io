    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header card-header-icon card-header-theme">
          ...
          <h4 class="card-title">Random Stock
            <span class="">
              <div class="btn-group header-frm-btn">
                @if(auth()->user()->can(['random_stock_download']))
                <form method="POST" action="{{url('random_stock/download')}}">
                  @csrf
                ...
                <div class="row next-btn">
                  @if(auth()->user()->can(['random_stock_upload']))
                  <form action="{{ URL::to('random_stock/upload') }}" class="form-horizontal" method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                  ...
                  @if(auth()->user()->can(['random_stock_template']))
                  <!-- primary sales template creation -->
                  <a href="{{ URL::to('random_stock_template') }}" class="btn btn-just-icon btn-theme" title="{!!  trans('panel.global.template') !!} Stock"><i class="material-icons">text_snippet</i></a>
                  @endif
                </div>
              </div>
            </span>
