    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($query) {
                  ...
                  {
                    $btn = $btn.'<a href"javascript:void(0)" class="btn btn-info btn-just-icon btn-sm edit" id="'.encrypt($query->id).'" title="'.trans('panel.global.edit').' '.trans('panel.brand.title_singular').'">
            ...
            ->addColumn('active', function ($query) {
              if(auth()->user()->can(['brand_active']))
              {
                ...
                return '<div class="togglebutton">
                    <label>
                      <input type="checkbox"'.$active.' id="'.$query->id.'" class="brandActive">
